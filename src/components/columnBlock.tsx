...
export default function ColumnBlock({width="lg:max-w-[370px]", theme, title, sub, icon}: BlockProps) {
    ...
    return (
        <div onMouseOver={() => setHoverState(true)} onMouseOut={() => setHoverState(false)} className={`${theme === 'white' ? 'text-white globalHoverInvert' : 'text-subText globalHover'} flex flex-col justify-end items-start h-[386px] lg:h-[500px] max-w-[351px] w-[95vw] md:max-w-[640px] ${width} py-[32px] px-[16px] rounded-none border-b-[1px] border-opacity-[0.6] border-${theme}`}>
            ...
            <p className={`${subFont.className} text-[18px] md:text-[20px] leading-[22px] ${theme === 'white' ? 'text-subTextInvert' : 'text-subText'} ${hoverState ? 'lg:text-subText' : 'lg:text-subTextInvert'} duration-150 text-opacity-80`}>
                {sub}
            </p>
        </div>
    )
